...
describe("parseConfig", () => {
	...
	test.each([
		...
		[{ version: 1, sources: ["../private.md"] }, "must stay within the project root"],
		[{ version: 1, sources: ["!private.md"] }, "must not use a negative glob"],
		[{ version: 1, sources: ["docs/*.md"], output: "*.md" }, "must not contain a glob"],
		[{ version: 1, sources: ["docs/*.md"], extra: true }, "Unknown configuration key"],
		[{ version: 1, sources: ["docs/*.md"], $schema: 1 }, "$schema must be a string"],
...
describe("loadConfig", () => {
	test("resolves paths from the configuration directory", async () => {
		await withTempDirectory(async (directory) => {
			...
			await writeText(configPath, JSON.stringify({ version: 1, sources: ["docs/*.md"] }));
	...
	test("reports invalid JSON as a domain error", async () => {
		await withTempDirectory(async (directory) => {
			...
			await expect(loadConfig(configPath)).rejects.toBeInstanceOf(AgentsComposeError);
		});
	});
});
